...
def test_extractor():
    """test extractor"""
    result = runner.invoke(cli.app, ["extract", "-i", "test_files/Metadata-test.wsdl", "-o", "output.wsdl", "-m", "Flow"])
    assert result.exit_code == 0
    # print(result.stdout)
...
def test_patch():
    """test extractor"""
    result = runner.invoke(cli.app, ["patch", "-e", "test_files/mapping-test.json", "-i", "test_files/MetadataServiceImported-test.cls", "-o", "MetadataService.cls", "-a", "58.0"])
    assert result.exit_code == 0
    # print(result.stdout)
